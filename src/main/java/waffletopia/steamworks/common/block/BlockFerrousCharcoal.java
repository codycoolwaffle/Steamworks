...
public class BlockFerrousCharcoal extends SWBlock {
	...
	public static final BooleanProperty IS_MULTIBLOCK_VALID = BooleanProperty.create("multiblock_valid");
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit){
		if(worldIn.isRemote()) return ActionResultType.PASS;
			...
			if(player.getHeldItem(handIn).isItemEqualIgnoreDurability(Items.FLINT_AND_STEEL.getDefaultInstance())){
	...
	public void validateMultiBlock(BlockState state, ServerWorld worldIn, BlockPos pos) {
		worldIn.setBlockState(pos, state.with(IS_MULTIBLOCK_VALID,
			  worldIn.getBlockState(pos.add(0, -2, 0)).isSolid()
			  && worldIn.getBlockState(pos.add(0, -1, 0)) == SWBlocks.blockCharcoal.getDefaultState()
	...
	public void tick(BlockState state, ServerWorld worldIn, BlockPos pos, Random random) {
		...
		if (random.nextInt(8) == 0) {
			...
			if (state.get(IS_BURNING)) {
				if (!state.get(IS_MULTIBLOCK_VALID)) {
					if (state.get(DAMAGE) >= 3) {
						worldIn.setBlockState(pos, SWBlocks.blockOxidizedIronBloom.getDefaultState());
					}
				} else {
					if (!isMaxAge(state)) {
			...
			} else if (state.get(IS_MULTIBLOCK_VALID)) {
				for (BlockState b : new BlockState[]{
					  worldIn.getBlockState(pos.add(-1, 0, 0)),
					  worldIn.getBlockState(pos.add(1, 0, 0)),
					  worldIn.getBlockState(pos.add(0, 0, -1)),
					  worldIn.getBlockState(pos.add(0, 0, 1))
				}) {
					if (random.nextInt(2) == 0 && b.get(IS_BURNING) && b.get(IS_MULTIBLOCK_VALID)) {
						worldIn.setBlockState(pos, state.with(IS_BURNING, true));
					}
				}
			}
